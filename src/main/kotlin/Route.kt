...
fun Application.routes() {
  for (route in Route.entries) {
    ...
    routing { get(path = route.path) { call.pageTemplate(route) } }
  }
}

suspend inline fun ApplicationCall.pageTemplate(currentPage: Route) = respondHtml {
  lang = "en"
  head {
    title { +"Aidan Pinard - ${currentPage.pageTitle}" }
    // Meta Tags
    ...
    link(
        href =
            "https://fonts.googleapis.com/css2?family=Fira+Code&family=Inter:ital,opsz@0,14..32;1,14..32&family=Libre+Baskerville:wght@700&display=swap",
        rel = "stylesheet",
    )
  ...
  body(
      "flex flex-col leading-relaxed min-h-dvh items-center text-lg text-primary bg-secondary dark:text-secondary dark:bg-primary",
  ) {
    header(" sticky z-50 bg-gray-300 top-0 p-4 w-full max-w-xl bg-secondary dark:bg-primary") {
      ...
      nav("flex flex-row space-x-3") {
        for (route in Route.entries) {
          linkButton(href = currentPage.sameRouteOrDefault(route.path)) { +route.pageTitle }
        }
      }
    }
    main("flex-grow w-full max-w-xl") { currentPage.content(this) }
    footer(
        "border-t-4 border-primary dark:border-secondary sticky z-50 bottom-0 p-4 w-full max-w-xl text-center bg-secondary dark:bg-primary",
    ) {
      +"Made with Kotlin :)"
    }
  }
...
inline fun SectioningOrFlowContent.customSection(
    title: String,
    crossinline block: SECTION.() -> Unit,
) =
    section("p-4") {
      h1("text-xl font-serif font-bold") { +title }
      block()
    }
...
fun FlowContent.about() {
  ...
  customSection("About this site") {
    p("pb-4") { +"This site was made using :" }
...
fun FlowContent.contact() {
  customSection("Contact Me!") {
    p("pb-4") { +"You can contact me (or see what I'm up to) via:" }
    ul("list-disc list-inside pb-4") {
      contacts.map { li { inlineLink(href = it.second) { +it.first } } }
    }
  }
}
...
inline fun FlowContent.linkButton(
    ...
    a(
        href,
        target,
        "$classes underline p-2 border-2 rounded border-primary dark:border-secondary",
        block,
    )
